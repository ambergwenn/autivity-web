...
export function ClassCreateDialog({
    ...
    // Reset form states whenever dialog opens
    React.useEffect(() => {
        if (open) {
            setTitle("")
            setGrade("Grade 1")
    ...
    const handleCreate = async (e: React.FormEvent) => {
        ...
        // Format schedule string matching DB structure e.g. "Monday - Wednesday 10:00 AM"
        const formattedSchedule = getFormattedSchedule(startDay, endDay, scheduleTime)

        const newClassData = {
            title: title.trim(),
            grade: grade.trim() || "Unassigned Grade",
        ...
        const res = await createClass(newClassData)
                ...
                <DialogHeader className="pb-3 border-b border-slate-100">
                    <div className="flex items-center gap-3">
                        <div className="flex size-12 items-center justify-center rounded-2xl bg-[#62A9E6]/15 text-[#2E79B9] font-bold text-lg font-fredoka shrink-0">
                            <Plus className="size-6 text-[#2E79B9]" />
                        </div>
                        <div>
                            ...
                            <DialogDescription className="text-xs font-semibold text-slate-500 mt-0.5">
                                Enter class details to save to the database
                            </DialogDescription>
                        </div>
                    </div>
                </DialogHeader>
